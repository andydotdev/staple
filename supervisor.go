...
type Supervisor struct {
	...
	// holding this in a struct, I think due to the function of staple
...
The name is a friendly human name for the supervisor, used in logging. Staple
...
The EventHook function will be called when errors occur. Staple will log the
following:
...
The failureRate, failureThreshold, and failureBackoff controls how failures are
handled, in order to avoid the supervisor failure case where the program does
nothing but restarting failed services. If you do not care how failures behave,
the default values should be fine for the vast majority of services, but if you
want the details:
...
exponential function.) When a failure occurs, the number of failures is
incremented by one. When the number of failures passes the FailureThreshold, the
entire service waits for FailureBackoff seconds before attempting any further
restarts, at which point it resets its failure count to zero.

Timeout is how long Staple will wait for a service to properly terminate.

The PassThroughPanics options can be set to let panics in services propagate and
crash the program, should this be desirable.

DontPropagateTermination indicates whether this supervisor tree will propagate a
ErrTerminateTree if a child process returns it. If false, this supervisor will
itself return an error that will terminate its parent. If true, it will merely
return ErrDoNotRestart. false by default.
*/
...
func (s *Supervisor) Add(service Service) ServiceToken {
	if s == nil {
		panic("can't add service to nil *staple.Supervisor")
	}
...
func (s *Supervisor) Serve(ctx context.Context) error {
	...
	if s == nil {
		panic("Can't serve with a nil *staple.Supervisor")
	}
	...
	if s.id == 0 {
		panic("Can't call Serve on an incorrectly-constructed *staple.Supervisor")
	}
...
func (s *Spec) configureDefaults(supervisorName string) {
	if s.FailureDecay == 0 {
		s.FailureDecay = DefaultFailureDecay
	}
	if s.FailureThreshold == 0 {
		s.FailureThreshold = DefaultFailureThreshold
	}
	if s.FailureBackoff == 0 {
		s.FailureBackoff = DefaultFailureBackoff
	}
	...
	if s.EventHook == nil {
		defaultLogger := slog.New(slog.NewTextHandler(os.Stderr, &slog.HandlerOptions{
			AddSource: false,
		})).With("supervisor", supervisorName)
		s.EventHook = func(e Event) {
			defaultLogger.Log(context.Background(), e.Level(), e.String())
		}
	}
